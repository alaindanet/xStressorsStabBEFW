...
date: "Proceeding B doi:10.1098/rspb.2025.0730"
output:
...
(ref:sem-sens-cap) Total standardised effects for the SEM displayed in main text (Fig. 1)
...
(ref:resp-cap) Effects of response diversity on compensatory effects and
portfolio effects in food web (a) and primary producer communities (b). Comp:
compensatory; Doak: Computation of portfolio effects according to Doak et al.
(1998) (reference in the main text). Black horizontal lines display the value of
1, i.e. when those asynchrony partitions have no effects on asynchrony and
therefore on community stability. As expected in pure competitive communities
(b) where all species are the same and so that community biomass are evenly
distributed, the compensatory effect due to the environmental stochasticity is
one when response diversity is one, i.e. when all species have independent
stochastic mortality rate. When stochastic mortality rates are correlated
(response diversity < 1), compensatory effects due to environmental
stochasticity and portfolio effects decrease.

```{r p-resp, fig.cap="(ref:resp-cap)"}
knitr::include_graphics(here("report", "figures", "p_resp_div_async_decomp.pdf"))
```

(ref:cor-cap) Spearman's correlation coefficients among stability components,
...
relationship between community stability and species richness (Fig. 2 in main text).

```{r coeff-lm, fig.cap="(ref:coeff-lm-cap)", out.width = "80%"}
...
relationship between community stability and species richness (Fig. 2 in main text). a,b,c
...
p_pred_stab_comp_rho_cap <- paste0("Prediction of the statistical model linking
  ...
  Fig. 2 in main text, we added the prediction of the slope of the relation population
...
tar_read(tab_sem_vif) %>%
  kable(
    ...
    displayed in main text (Fig. 1). All VIF were inferior to 3, indicating low
    multicollinearity.",
    label = "sem-vif"
    ) %>%
  column_spec(1, bold = TRUE) %>%
...
tar_read(tab_model_fw_stab_comp_wo_int_vif) %>%
  kable(booktabs = TRUE,
    ...
    relationship between community stability and species richness (Fig. 2 in main text). All
