...
Empirical and theoretical evidence suggests that current and projected
environmental changes and biotic homogenisation will dramatically destabilise
ecological communities. However, this evidence is largely based on
oversimplified communities (i.e. food chains, guild assemblages), which lack the
complexity of natural communities.We characterise the dynamics of complex
food-webs under a stochastic bioenergetic food-web model, and investigate how
response diversity, environmental stochasticity, and food-web structure drive
temporal stability. We found that higher average trophic level, omnivory degree
and generalism increased average population stability, but decreased asynchrony,
triggering compensatory dynamics that raised trophic interactions.  Species
richness had little effect on average population stability, but a large one on
synchrony. Further, when response diversity is low, high interaction strength,
average trophic level, and connectance all lead to negative stability-species
richness relationships, but when response diversity is high, we observe positive
stability-species richness relationships regardless of the food-web structure.
Our study reveals that response diversity and food-web structure drive temporal
...
and functioning in these communities. Assessing the impacts of this increased
variability on ecological communities is challenging for several reasons.

First, the stability of communities is often evaluated using the temporal
variability of a metric of interest (e.g. abundance or biomass), because other
stability metrics such as resilience and resistance are not always applicable,
as most natural communities face persistent perturbations [@arnoldi_resilience_2016].
Understanding this temporal community variability (hereafter referred to as
community stability), measured as the inverse of the coefficient of variation
[@tilman_biodiversity_2006; @roscher_identifying_2011;
@thibaut_understanding_2013; @zhao_biodiversity_2022], requires scaling down the
focus from communities to populations.
Community stability can be mathematically partitioned into average population
stability and asynchrony [@loreau_species_2008;
@thibaut_understanding_2013; @zhao_biodiversity_2022] - asynchrony measuring how
much each species fluctuations’ are compensating for each other at the community
level. Population stability and asynchrony both depend on the number of species
and their dominance [@loreau_species_2008; @thibaut_understanding_2013;
@zhao_biodiversity_2022], on the interactions among species [@thebault_trophic_2005;
@thebault_relationship_2006; @loreau_species_2008; @danet_species_2021;
@eschenbrenner_diversity_2023] and on the way species respond to environmental
perturbations [@ives_stability_2000; so-called response diversity,
@elmqvist_response_2003; @ross_how_2023]. Therefore, accounting for the effects
of community structure on population stability and asynchrony is crucial to
improve our understanding of community stability in facing environmental change.

Average population stability decreases with environmental stochasticity, such as the
intensity and the frequency of perturbations [@loreau_species_2008]. Theorerical
and empiral evidence suggests that species richness can decrease
[@tilman_biodiversity_2006; @loreau_species_2008], increase
...
the community since dominant species contribute more to the average of the
community than subordinate species. Species interactions also have tremendous
...
@thebault_trophic_2005]. The shape of food-webs can also modulate population
stability, such as higher trophic level was generally found to be stabilising
[@shanafelt_stability_2018; @danet_species_2021; @eschenbrenner_diversity_2023],
as top-down and bottom-up controls can increase population stability
[@shanafelt_stability_2018].

Asynchrony is mostly determined by the characteristics of the network. Response
diversity captures how species’ differences in their response to the
environmental variation can lead to asynchronous dynamics if their responses are
different but synchronous dynamics if their responses are similar. Theory and
empirical data emerging around response diversity suggest that it is a “core
ecological mechanism underpinning many of the statistical explanations of the
diversity–stability relationships” [@ives_stability_2000;
@elmqvist_response_2003; @ross_how_2023]. In turn, the network structural
properties and strength of species interactions can also modulate asynchrony
through compensatory dynamics such as a strong asymmetry of consumer preference
can generate asynchrony in resource fluctuations [@mccann_weak_1998;
...
affected by the population evenness such as fluctuations by dominant species
will hardly be compensated by a subordinate species
[@thibaut_understanding_2013; @zhao_biodiversity_2022]. Improving our
understanding of community stability facing perturbations therefore requires
articulating the effects of response diversity, species interactions, species
richness and evenness on the different components of stability. Predicting the
effects of species richness on community stability thus requires understanding
the relationship between species richness, population stability and asynchrony.

Empirical evidence suggest by increasing asynchrony, species richness
drives positive relationships between community stability and species richness
[@tilman_biodiversity_2006; @isbell_biodiversity_2009]. In contrast population stability was
found to either decrease, increase, or not change with species richness
[@houlahan_negative_2018; @tilman_biodiversity_2006; @roscher_identifying_2011].
We note, however, that evidence for both asynchrony-richness and population
stability-richness is derived from data on species assemblages.  These data thus
do not account for food-web structure, which can have significant impact on
population stability and asynchrony [but see @danet_species_2021]. Thus, ecology
lacks a body of theory of temporal stability integrating across structure and
response diversity frameworks and reflecting species rich communities. Here, we
integrate these two perspectives on diversity- stability to evaluate the
combined effects of community structure and response diversity on temporal
stability in species rich, complex communities characterised by competition and
predation.

The ability to do this is supported by a key statistical advance: it is now
possible to partition asynchrony into two component sources: the portfolio
effects (rising from statistical averaging effects, SAE) and the compensatory
dynamics effect (CPE) [@zhao_biodiversity_2022]. Portfolio effects stem from the
statistical averaging of independent population fluctuations [@doak_statistical_1998;
@zhao_biodiversity_2022], thereby increasing asynchrony. Compensatory dynamics
in turn capture population fluctuations that are more synchronous or
asynchronous than expected for independent fluctuations, and can stem from
species interactions and species responses to perturbations [@zhao_biodiversity_2022].
This allows an unprecedented opportunity to obtain mechanistic insight about the
effects of stressors and food-web structure on stability in complex, species
rich communities.

Our primary objective is to derive theory that predicts how the components of
community stability, that can differentially drive patterns of stability,
deliver novel insights into the source and patterns of diversity-stability
relationships in complex communities facing environmental stochasticity, a
necessary ingredient to generate species fluctuations. Our approach delivers
revised insight into diversity-stability relationships in species rich
communities when both community structure per se, and species responses can
vary. In the following sections, we begin by introducing the multi-species model
and the features of the model that allow us to integrate across variation in
community structure and response diversity. We then introduce assumptions we
used to implement environmental stochasticity, response diversity, variation in
community structure and variation in the distribution of interaction types and
strengths. Next, we define our in-silico experimental design that specifically
aligns with three core questions and associated analyses. Considering four
drivers, namely food-web structure, interaction strength, environmental
stochasticity and response diversity, we investigated (1) What are their direct
effects on population stability, portfolio and compensatory effects ? (2) What
are their total effects on community stability, population stability and
asynchrony? Finally, merging the two parallel bodies of work, we asked (3) How
do they interact to drive the sign of the relation between community stability
...
and varied predator interference (i.e. from $c = `r pc["min"]`$ to $c = `r
pc["max"]`$, eq. \@ref(eq:holling)), as predator interference was shown to have
a tremendous role on population stability [@brose_allometric_2006]. We set a global
carrying capacity of $K' = 10$ to ensure that consumers were not
...
disconnected species, we re-ran simulations with species starting biomass equals
...
Factor < 3, Table S1), altough interaction strength and species richness were
...
SEM to obtain confidence interval on the estimated effects and computed the sum
...
were inferior to 3, indicating low multicollinearity (Table S2), we checked the
...
```{r}
sem_cap <- paste0(
  "Structural Equation Model food-web struture and stressors to temporal
  ...
  displayed in Table S3. A more complete SEM containing further partition of
  compensatory effects and control variables is displayed in Table S4. (b) Total
  effect of food-web structure on community stability, asynchrony, and
...
positive effect ($r_\delta$= `r rci`, Fig.
\@ref(fig:sem)).
...
community stability ($r_\delta$=`r rtsr`), steming from its strong total positive effect on
asynchrony ($r_\delta$=`r rtar`) and downplayed by its small total negative effect on population
stability ($r_\delta$=`r rtpr`), but the fact that population stability effect on community
...
One of the key question in Ecology is what drive temporal stability in complex
...
fluctuations. We found that that food-web structure, apart from species richness,
...
lower mortality rates with in turn makes them less sensible stochastic
...
suggesting that average omnivory also stabilise population stability, in line
...
average omnivory, which might tamper synchronized predator-prey oscillations
...
had a strong positive on asynchrony, by raising portfolio effects.
...
the generalism in food-webs, had small negative effects population
stability but small positive on asynchrony. We further found that higher connectance
leaded to less even biomass variability, thereby limiting the raise of the
portfolio effects, and leaded to lower compensatory dynamics due to species
interactions (Table. S4). This result is in accordance with empirical findings
that a generalist predator can indeed synchronises the dynamics of a large
number of preys [@raimondo_interspecific_2004], thereby potentially limiting compensatory
dynamics raising from species interactions. However, the overall effect of
...
strong effects on community stability. Our results confirm the stabilising
...
fluctuation of prey biomass. All toghether, this result suggest that
self-limitation mechanisms that increase population stability might in turn have
opposite effect on asynchrony, and that it is the balance between the two
...
stability-richness relationship are cancelled when response diversity is high.
An possible explanation is the presence of response diversity and resulting
...
one mechanism that explains why positive relationship between stability
and species richness are so prevalent in empirical settings while food-web
theoretical model that do not often include environmental stochasticity and
...
population fluctuations. In turn, our study confirm that species richness
increase temporal community stability by increasing asynchrony due to portfolio
...
our study reinforce that the idea that restoration of ecosystems along with a
