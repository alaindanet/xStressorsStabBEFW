...
@thibaut_understanding_2013; @zhao_biodiversity_2022], requires to scale down
the focus to populations. Community stability can be mathematically partitionned
...
and their dominance [@loreau_species_2008; @thibaut_understanding_2013; @zhao_biodiversity_2022],
the interactions among them [@thebault_trophic_2005;
@thebault_relationship_2006; @loreau_species_2008; @danet_species_2021;
@eschenbrenner_diversity_nodate] and on the way that species respond to
...
intensity and the frequency of perturbations [@ref]. Species richness can
...
community than surbordinate species. At the core of ecological theory,
population stability depends on internal forcing, here species interactions. The
topogical structure and the distribution of interaction strength have tremendous
...
Asynchrony in turn can be partitionned into portefolio effects and compensatory
dynamics and is mostly determine by internal factors. Portefolio effects stems
...
are more synchronous or asynchronous than expected for independant fluctuations,
...
asynchrony through compensatory dynamics such as a strong asymetry of consumer
...
[@loreau_species_2008]. Finally, portefolio and compensatory effects can also be
affected by the population evenness such as fluctuations of a dominant
species will be hardly compensated by a subordinate species. Improving our
...
then introduce assumptions we use to implement environmental stochasticity,
...
and statistical averaging effects (i.e. portefolio effects)? (2) What are the
total effects of food-web structure, reponse diversity, environmental
...
models [@vasseur_environmental_2007], we assumed that
...
with an initial species richness from $10$ to $60$, and connectance from $0.02$
to $.32$. We generated food-webs without cannibalistic links and discarded
...
varied the functional response of consumer from type II to type III (resp. $h=2$
and $h=3$, eq XX) as it was shown to have a tremendous role on stability
[@brose_allometric_2006]. We set $K = 10$ to ensure that consumers were not
limited by the biomass input from primary producers.
...
We numerically solved the simulations during 5000 timesteps
and collected the last 500 timesteps. We set $d_t = .1$, and $d_t = .05$ when
instability was detected during the simulations. We set extinction threshold to
...
events. When we found disconnected species at the end of the simulation (i.e. a
primary producer without consumer or a consumer without prey), we removed them
and re-run simulation for another 1000 timesteps.
...
As SAE is further dependant on the evenness of species variance, we
partitioned $SAE$ into $SAE_{even}$ and evenness $EVN$, such as $SAE_{even} =
...
average trophic level, connectance, hill exponent, predator-prey size ratio, and
...
multicollinearity. We then used the coefficients of the model to predict the shape
of the relationship between community stability and species richness according
to a gradient of response diversity and of food-web structure.
...
sem_cap <- paste0(
  ...
  displayed in Fig. SXX. (b) Partial relationships derived from the SEM, points
  displaying the partial standardized residuals.")
```

```{r sem, fig.cap=sem_cap, fig.id = "sem"}
knitr::include_graphics(here("report", "figures", "p_sem_tot.pdf"))
```
...
`r rph`, $r_\delta$ being the standardized coefficient, Fig. \@ref(fig:sem)).
...
`r rsi`), thereby downplaying the portfolio effect raised from species
...
\@ref(fig:sem)). Species response diversity had a strong positive effect on compensatory dynamics
($r_\delta$= `r rcd`).
...
```{r semtot, fig.cap = semtot_cap, fig.id = "semtot"}
...
$r_\delta$= `r rtsenv` and `r rtsdiv`, Fig. \@ref(fig:semtot)). Species richness
...
predator prey bodysize ratio had positive effects on community stability in
total (resp. `r rtst`, `r rtsh`, `r rtsz`, Fig. \@ref(fig:semtot)), driven by
...
`r rtsc` and `r rtsi`, Fig. \@ref(fig:semtot)). While connectance had negative
...
Using a more complex linear model (i.e. including interactions), we investigated
more in detail the relationship between community stability and species richness
and how this relation is affected by environmental stochasticity and species
response diversity. We found that environmental stochasticity decrease overall
community stability but has only little effect on the shape of the community
stability-species richness relationship (put some numbers, Fig. \@ref(fig:pred)a
and b). In contrasts, species response diversity both increases overall
community stability and leads to positive community stability-species richness
relationships. Surprisingly, we found that despite average interaction strength
has a small overall effect on community stability, high average interaction
strength change the shape of the community stability-species richness
relationship from positive to negative when response diversity is low (put some
numbers, Fig. \@ref(fig:pred)b).
...
data in birds and bats assemblages that reported a equivalent influence of
...
food-webs, which typically include species that differ by several order of
...
fluctuations (i.e. portfolio effects) and compensatory dynamics. The question of
which mechanisms and community structure features drive differences in the
contribution of asynchrony and population stability in food-webs and competitive
assemblages is still a open question that might be crucial to accurately
address diagnostics and conservation measures. On top of food-web structure,
we found that environmental stochasticity and environmental correlation among
species was the main drivers of community stability, suggesting that stressors
are the first source of stability. This result is in accordance with previous
...
stability, confirming previous theoreical [@brose_allometric_2006;
@shanafelt_stability_2018; @eschenbrenner_diversity_nodate] and
...
prey [@shanafelt_stability_2018, we can test that?]. In turn, we found that that
...
[@vasseur_environmental_2007], stabilising populations and simultaneously
synchronising them. The fact that Predator Size Mass Ratio displays the same
...
statistical averaging effects. [add blabla about richness asynchrony]. In turn,
...
that higher species richness synchronizes more species fluctuations due to
...
structures. Connectance, quantifying generalism in food-web, had negative
effects on both asynchrony and population stability. We further found that
higher connectance leaded to less even biomass variability, thereby limiting the
raise of the portfolio effects, and leaded to lower compensatory dynamics due
to species interactions. This results in accordance with empirical finding that
a generalist predator can indeed synchronises the dynamics of a large number of
preys [@raimondo_interspecific_2004]. [No idea about connectance decreasing
population stability]. However, the overall effect of connectance on overall
stability was quite low, similarly to previous empirical findings
[@danet_species_2021], but in apparent disagreement with the predominant role of
connectance on stability in theoretical studies [complexity-stability paradox,
@may_will_1972].

Food-web dynamical features, functional response of type III and average
interaction strength, also had an effect on community stability. Our results
confirms the stabilising effect of the functional response of type III on
...
fluctuations of prey biomass. Furthermore, we found that functional response of
type III decreased asynchrony by decreasing compensatory dynamics due to species
interaction. This effect might be due to prey switching of consumers when one of
...
evenness of biomass variability, thereby decreasing the portfolio effect,
...
stability (i.e. the intercept of the relation). At the opposite, In turn, a
decrease in response diversity dampens the stability-richness relationship. The
reason is that the strength of environmental stochasticity only affect
population stability and not asynchrony, the latter being the driver of
...
decrease as the community is species diversity, has higher connectance, and
...
when response diversity is high. A interpretation is that environmental
...
complexity on stability-richness relationship. Response diversity might then one
...
food-web, added to two pressing stressors in the context of global change, the
...
level. Then, our study reinforces that the restoration of ecosystems along with
a restoration of landscape complexity that provides refuges from extreme
environment are key to maintain temporal stability of ecological communities
[@ryser_landscape_2021].
