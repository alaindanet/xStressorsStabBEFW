...
The consumers gain biomass by consumming resources (i.e. their preys) at a rate
that depends on their mass specific metabolic rate ($x_i$),  maximum cumsumption
...
Consumers then loses biomass by being consumed, $e_ij$ being the assimilisation
...
of the consumer on the resource ($\omega_ij$) is limited by an half-saturation
...
(Holling type II) to a sigmoid (Holling type III).
...
`r rph`, $r_\delta$ being the standardized coefficient, Fig. \@ref(fig:sem)a).
...
food-webs, which typically include species that differ in bodymass by several order of
