...
variability can be partitioned into population stability and asynchrony
...
stability while compensatory dynamics and portfolio effects (i.e. asynchrony)
...
on the diversity of species responses to environmental fluctuations [response
diversity, @ives_stability_2000; @elmqvist_response_2003]. Increasing
species richness has been reported to decrease population stability in
experimental grasslands [@tilman_biodiversity_2006; @roscher_identifying_2011],
...
averaging effects due to independent species fluctuations) and compensatory
...
We numerically solved the simulations with a $dt$ = .1 during 5000 timesteps
...
effects of species richness and species cross-correlation in mortality rates on
...
\@ref(fig:sem)). Cross-correlation among species mortality rates (i.e. lower
response diversity) had a strong negative effect on compensatory dynamics
($r_\delta$= `r rco`), meaning that the loss of response diversity result in a
strong decrease in compensatory dynamics.
...
```{r}
rtsrho <- get_eff(y = "stab_com", x = "rho", effect_type = "total")
...
model, we show that both environmental stochasticity and cross-correlation among
species had by far the most negative effect on community stability (resp.
$r_\delta$= `r rtsenv` and `r rtsrho`, Fig. \@ref(fig:semtot)). Species richness
...
the community stability-species richness relationship. Surprisingly, we found that
