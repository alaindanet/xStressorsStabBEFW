...
effects ($r_\delta$= `r rsd`), but also through a weaker positive effect on compensatory
...
species have correlated responses.
...
direct standardised coefficients that had an absolute value above $0.05$,
because almost all linked were statistically significant given the high numbers
of simulations.
...
\@ref(fig:pred). The VIF were inferior to 3, indicating low multicollinearity
(Table S5), we checked the distribution of the residuals (Fig. S6) using
`DHARMa` R package. The model was implemented using the R package
`glmmTMB`[@brooks_glmmtmb_2017].
