...
tar_read(tab_sim_param) %>%
  kable(
    ...
    brackets are the minumum and maximal values, N being the number of unique
...
Bodysize Ratio (PPBR) from $`r pZ["min"]`$ to $`r pZ["max"]`$ (Fig. \@ref(fig:met)b). We further varied
the functional response of consumer from type II to type III (resp.
$`r ph["min"]`$ to $`r ph["max"]`$, eq. \@ref(eq:holling)) as it was shown to
...
community. Species trophic level was computed recursevily from the bottom of the
...
consumer was computed  such as the sum of square of its
resource trophic levels weighted by the relative preference of the consumer for
this resource: $\sum_i (\mathrm{TL}_i - \hat{\mathrm{TL}})^2 \omega_i/ \sum \omega_i$ [@christensen_ecopath_1992].
...
consumer. Finally, the average interaction strength of a community was computed
as the average per capita interaction strength at the exclusion of null
interaction strengths (i.e. non present trophic interactions).
