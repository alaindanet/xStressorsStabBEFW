...
list(
  tar_target(sem_model_list,
    list(
      ...
      w_avg_tlvl = lm(w_avg_tlvl ~ richness + ct_alive, sim_fw_df),
      avg_omnivory = lm(avg_omnivory ~ w_avg_tlvl + ct_alive + richness, sim_fw_df),
      avg_int_strength = lm(avg_int_strength ~ w_avg_tlvl + ct_alive + richness + h, sim_fw_df),
      pop_stab = lm(pop_stab ~ Z + richness + avg_int_strength + avg_omnivory + w_avg_tlvl + ct_alive + env_stoch + h, sim_fw_df),
      ...
      cpe_int = lm(cpe_int ~ richness + avg_int_strength + avg_omnivory + w_avg_tlvl + ct_alive + h, sim_fw_df),
...
#      theme_bw()),
    tar_target(p_stab_s_env_stoch,
      sim_fw %>%
        filter(env_stoch %in% c(.5, 1.5), rho == 0, productivity == 5) %>%
...
summary(lm(cpe_env ~ richness+ct_alive+avg_omnivory+rho, sim_fw))
summary(lm(cpe_int ~ richness+ct_alive+avg_omnivory+rho, sim_fw))
summary(lm(cpe ~ richness+ct_alive+avg_omnivory+rho, sim_fw))
performance::check_collinearity(lm(cpe_env ~ richness+ct_alive+avg_omnivory+rho, sim_fw))
mod_cpe <- lm(cpe ~ avg_int_strength + avg_omnivory + w_avg_tlvl + ct_alive + productivity + rho, sim_fw)
summary(mod_cpe_int)
performance::check_collinearity(mod_cpe_int)

mod_cpe <- lm(log(cpe) ~ avg_int_strength + avg_omnivory + w_avg_tlvl + ct_alive + productivity + rho, sim_fw)
...
sim_fw_cons %>%
  filter(env_stoch == .6, rho == 1, Z == 100, productivity == 5) %>%
  ggplot(aes(x = richness, y = stab_com, color = as.factor(h))) +
  geom_point() +
  geom_smooth()

sim_fw %>%
  filter(env_stoch == .6, rho == 0, Z == 100, productivity == 15) %>%
  ggplot(aes(x = richness, y = async, color = as.factor(h))) +
  geom_point() +
  geom_smooth()

sim_fw_cons %>%
  filter(env_stoch == .6, rho == 0, Z == 100, productivity == 30) %>%
  ggplot(aes(x = richness, y = pop_stab, color = as.factor(h))) +
  geom_point() +
  geom_smooth()
```

```{r}
plot(richness ~ Z, sim_fw)
```
...
p_pred_sr_ct_int <- pred_data %>%
  ...
  theme_bw()
ggsave(
  here("report", "figures", "p_pred_sr_ct_int.png"),
  plot = p_pred_sr_ct_int,
  scale = 2.4,
  width = 88,
  height = 88 * 1,
  units = "mm"
)
p_pred_sr_omn <- pred_data %>%
  filter(
    productivity == 5,
    ct_alive %in% c(.1),
    rho %in% c(0, 1),
    avg_int_strength %in% .02,
    avg_omnivory %in% c(.01, .1),
    w_avg_tlvl == 1.7) %>%
  ggplot(aes(x=richness, y = stab_com, color = as.factor(avg_omnivory))) +
  geom_line() +
  facet_grid(cols = vars(env_stoch), rows = vars(rho), labeller = label_both) +
  theme_bw()
ggsave(
  here("report", "figures", "p_pred_sr_omn.png"),
  plot = p_pred_sr_omn,
  scale = 2.4,
  width = 88,
  height = 88 * 1,
...
p_pred_sr_avg_tlvl <- pred_data %>%
  filter(
    productivity == 5,
    ct_alive %in% c(.1),
    rho %in% c(0, 1),
    avg_int_strength %in% .02,
    avg_omnivory %in% c(.05),
    w_avg_tlvl == c(1.5, 2)) %>%
  ggplot(aes(x=richness, y = stab_com, color = as.factor(w_avg_tlvl))) +
  geom_line() +
  facet_grid(cols = vars(env_stoch), rows = vars(rho), labeller = label_both) +
  theme_bw()
ggsave(
  here("report", "figures", "p_pred_sr_avg_tlvl.png"),
