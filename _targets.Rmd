...
list(
  tar_target(sem_simplified_model_list,
    list(
      ...
      avg_int_strength = lm(avg_int_strength ~ ct + S + h + Z, sim_fw_sem),
      pop_stab = lm(pop_stab ~ richness + avg_int_strength + w_avg_tlvl + ct_alive + env_stoch + h + Z, sim_fw_sem),
      sae_total = lm(sae_total ~ richness + avg_int_strength + w_avg_tlvl + ct_alive + h + Z, sim_fw_sem),
      cpe = lm(cpe ~ richness + avg_int_strength + w_avg_tlvl + ct_alive + h + Z + rho, sim_fw_sem),
...
list(
  ...
  tar_target(pred_sr_rho_int,
    list(
      rho = seq(0, 1, .02),
      avg_int_strength = seq(0.01, 0.2, .01)
      ) %>%
    expand.grid() %>%
    mutate(
...
# plot just the interactions
# TODO: order by values
ti %>%
...
tar_load(sim_fw)
colnames(sim_fw)
length(unique(sim_fw$fw_id))
```

## Modelling interaction between stability and food-web structure

```{r}
model_fw_stab <- glmmTMB(formula_stab_fw_model(
    rand_effect = TRUE,
    term_to_del = "richness : rho : Z +"),
  data = mutate(sim_fw, across(c(stab_com, richness), log))
)
diagnose(model_fw_stab)

model_fw_stab_scaled <- glmmTMB(formula_stab_fw_model(rand_effect = TRUE),
  data = sim_fw %>%
    select(-productivity) %>%
    mutate(across(c(stab_com, richness), log)) %>%
    mutate(across(c(sim_id, fw_id), as.factor)) %>%
...
fixef(model_fw_stab_scaled)
diagnose(model_fw_stab_scaled)
```
