...
```{r}
pred_sr_rho_tlvl %>%
  ggplot(aes(x = rho, y = w_avg_tlvl)) +
  geom_raster(aes(fill = slope_richness)) +
...
```{r}
p_pred_sr_rho_int <- tar_read(pred_sr_rho_int) %>%
  ggplot(aes(x = rho, y = avg_int_strength)) +
  geom_raster(aes(fill = slope_richness)) +
  scale_fill_gradient2(
    low = "red",
    mid = "white",
  ...
  labs(
    x = "Environmental correlation",
    y = "Avg interaction strength",
    fill = "Slope Stability-Richness"
  ) +
theme(legend.position = "bottom")
ggsave(
  here("paper", "figures", "p_pred_sr_rho_int.pdf"),
  plot = p_pred_sr_rho_int,
  scale = 2.4,
  width = 88,
  height = 88 * 1,
  units = "mm"
