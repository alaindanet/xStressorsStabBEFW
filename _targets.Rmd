...
Near the top of the document, you may also wish to remove the `_targets_r` directory
previously written by non-interactive runs of the report. Otherwise, your pipeline may
contain superfluous targets.
...
tar_option_set(
  error = "null",
  packages = c(
"tidyverse", "magrittr", "cowplot", "here", "arrow", "piecewiseSEM",
"semEff", "easystats", "glmmTMB", "ggcorrplot", "kableExtra", "DHARMa", "janitor",
"magick", "pdftools", "gridGraphics"
)
)
...
Get total sims for some checking about producer communities:

```{targets global-sim}
list(
  tar_target(sim_total_files, here::here("data", "sim"), format = "file"),
  tar_target(sim, readRDS(sim_total_files))
)
```
...
```{targets p-resp-div-async-decomp}
list(
  tar_target(resp_div_async_decomp_fw,
    sim_fw %>%
      select(richness, resp_div, cpe_env, sae_doak) %>%
      pivot_longer(cpe_env:sae_doak, names_to = "async_decomp", values_to = "values") %>%
      mutate(async_decomp = recode_factor(async_decomp, !!!var_replacement()))
    ),
  tar_target(resp_div_async_decomp_prod,
    sim %>%
      filter(max_tlvl == 1) %>%
      mutate(
        resp_div = 1 - rho,
        sae_doak = cpe_env * sae_total
        ) %>%
      select(richness, resp_div, cpe_env, sae_doak) %>%
      pivot_longer(cpe_env:sae_doak, names_to = "async_decomp", values_to = "values") %>%
      mutate(async_decomp = recode_factor(async_decomp, !!!var_replacement()))
    ),
  tar_target(p_resp_div_async_decomp_fw,
    resp_div_async_decomp_fw %>%
      ggplot(aes(x = as.factor(resp_div), y = values)) +
      geom_boxplot() +
      geom_hline(yintercept = 1) +
      facet_wrap(~async_decomp) +
      theme_bw() +
      labs(
        y = "Asynchrony partition",
        x = var_replacement()["resp_div"]
      )
    ),
  tar_target(p_resp_div_async_decomp_prod,
    resp_div_async_decomp_prod %>%
      ggplot(aes(x = as.factor(resp_div), y = values)) +
      geom_boxplot() +
      geom_hline(yintercept = 1) +
      ylim(c(min(resp_div_async_decomp_fw$values), max(resp_div_async_decomp_fw$values))) +
      facet_wrap(~async_decomp) +
      theme_bw() +
      labs(
        y = "Asynchrony partition",
  ...
  tar_target(p_resp_div_async_decomp,
    plot_grid(
      p_resp_div_async_decomp_fw,
      p_resp_div_async_decomp_prod,
      labels = "auto"
    )
    ),
  tar_target(p_resp_div_async_decomp_file,
    ggsave_multiple(
      paste0("p_resp_div_async_decomp", c(".png", ".pdf")),
      plot = p_resp_div_async_decomp,
      path = here::here("report", "figures"),
      scale = 2.4,
      dpi = 400,
      width = 80,
...
list(
  ...
  sim_summary_stat %>%
    ungroup() %>%
    mutate(
      var = recode_factor(var, !!!var_replacement(), .ordered = TRUE),
...
```{targets render, eval = TRUE}
# does not work properly, have to run it by hand
list(
  tar_render(manuscript, here::here("paper","manuscript.Rmd")),
  tar_render(supp, here::here("paper","supplementary_figures.Rmd"))
)
```
...
library(targets)
tar_load_globals()
error_tab <- tar_meta(fields = error, complete_only = TRUE)
error_tab |>
  filter(name == "resp_div_async_decomp")
tar_load(resp_div_async_decomp_prod)
```
...
sd(sim_fw_sem$rho) / sd(sim_fw_sem$env_stoch)
sd(sim_fw_sem$pop_stab) / sd(sim_fw_sem$async)
