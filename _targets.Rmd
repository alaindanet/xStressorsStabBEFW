...
tar_option_set(
  error = "null",
  packages = c("tidyverse", "magrittr", "cowplot", "here", "arrow", "piecewiseSEM", "semEff", "easystats", "glmmTMB", "ggcorrplot", "kableExtra")
)
...
tar_map(
  ...
  tar_target(sem_main_text_coeff, { readRDS(sem_main_text_coeff_file) }),
  tar_target(sem_main_text_eff_tab, from_semEff_to_table(sem_main_text_eff))
)
```

```{targets group-all-analysis, eval = FALSE}
list(
  tar_target(sem_main_text_tot,
    tibble(
      ...
      rsq = list(
        sem_main_text_rquared_csc_rerun,
        sem_main_text_rquared_csc_non_allo,
        sem_main_text_rquared_csc_rebuild,
        sem_main_text_rquared_csc_kno
        ),
      collinearity = list(
...
```{r, eval = FALSE}
tar_load(sem_main_text)
tar_load(sim_fw_sem)

m <- list(
  pop_stab = lm(pop_stab ~ richness + avg_int_strength + w_avg_tlvl + ct_alive + env_stoch + c + Z, sim_fw_sem),
  evenness_sae = lm(evenness_sae ~ richness + avg_int_strength + w_avg_tlvl + ct_alive + c + Z, sim_fw_sem),
  sae_total = lm(sae_total ~ richness + evenness_sae, sim_fw_sem),
  cpe = lm(cpe ~ richness + avg_int_strength + w_avg_tlvl + ct_alive + resp_div + c + Z, sim_fw_sem),
  async = lm(async ~ sae_total + cpe, sim_fw_sem),
  stab_com = lm(stab_com ~ pop_stab + async, sim_fw_sem)
)

m2 <- list(
  pop_stab = lm(pop_stab ~ richness + avg_int_strength + w_avg_tlvl + ct_alive + env_stoch + c + Z + resp_div, sim_fw_sem),
  evenness_sae = lm(evenness_sae ~ richness + avg_int_strength + w_avg_tlvl + ct_alive + c + Z + env_stoch + resp_div, sim_fw_sem),
  sae_total = lm(sae_total ~ richness + evenness_sae, sim_fw_sem),
  cpe = lm(cpe ~ richness + avg_int_strength + w_avg_tlvl + ct_alive + resp_div + c + Z + env_stoch, sim_fw_sem),
  async = lm(async ~ sae_total + cpe, sim_fw_sem),
  stab_com = lm(stab_com ~ pop_stab + async, sim_fw_sem)
)
...
m3 <- list(
  ...
  evenness_sae = lm(evenness_sae ~ richness + avg_int_strength + w_avg_tlvl + ct_alive + c + Z + env_stoch + resp_div, sim_fw_sem),
  sae_total = lm(sae_total ~ richness + evenness_sae, sim_fw_sem),
  cpe_int = lm(cpe_int ~ richness + avg_int_strength + w_avg_tlvl + ct_alive + resp_div + c + Z + env_stoch, sim_fw_sem),
  cpe_env = lm(cpe_env ~ richness + avg_int_strength + w_avg_tlvl + ct_alive + resp_div + c + Z + env_stoch, sim_fw_sem),
  #cpe = lm(cpe ~ cpe_env + cpe_int, sim_fw_sem),
  async = lm(async ~ sae_total + cpe, sim_fw_sem),
  stab_com = lm(stab_com ~ pop_stab + async, sim_fw_sem)
)
summary(as.psem(m3))
...
list(
  ...
  tar_target(model_fw_stab_comp_scaled,
    tibble(
      ...
      data = sim_fw %>%
        mutate(across(c(stab_com, richness, pop_stab, async), log)) %>%
      ...
      left_join(
        ...
        left_join(
          tibble(
            variable = c("avg_int_strength", "w_avg_tlvl", "ct_alive"),
            values = list(
              seq(quantile(sim_fw$avg_int_strength)["25%"], quantile(sim_fw$avg_int_strength)["75%"], length.out = 50),
              seq(quantile(sim_fw$w_avg_tlvl)["25%"], quantile(sim_fw$w_avg_tlvl)["75%"], length.out = 50),
              seq(quantile(sim_fw$ct_alive)["25%"], quantile(sim_fw$ct_alive)["75%"], length.out = 50)
            )
          )
        )
        ) %>%
...
list(
  ...
  tar_target(sem_file, here("report", "figures", "sem_stab_simplified_simple.pdf"), format = "file"),
  tar_target(p_sem,
    ggdraw() +
...
```
```{targets p_cor_file, tar_simple = TRUE, format = "file"}
ggsave_multiple(
...
list(
  ...
  tar_target(tab_sim_param,
    sim_param_tab %>%
      ...
      filter(!Parameter %in% c("sim_id", "fw_id", "rho")) %>%
      select(Parameter, Values) %>%
      mutate(Parameter = recode_factor(Parameter, !!!var_replacement()))
  ),
  ...
  tar_target(param_table,
    fix_parameter %>%
      ...
      mutate(
        ...
        Parameter = surround_latex_math(Parameter)
      ) %>%
...
```{r, eval=FALSE}
#tar_load_globals()
tar_load(sem_main_text_eff_tab)
sem_main_text_eff_tab %>%
  filter(
    effect_type == "direct",
    predictor %in% c(sem_tot_predictor(), "async", "stab_com", "pop_stab", "sae_total", "cpe", "evenness_sae", "sae_even")
    ) %>%
select(response, predictor, effect) %>%
mutate(effect = round(effect, 2)) %>%
filter(abs(effect) > .02)
```

```{r}
tar_load(sem_main_text_collinearity_csc_rerun)
sem_main_text_collinearity_csc_rerun %>%
  select(response, Term, VIF) %>%
  filter(VIF > 3)
```

```{r}
tar_load(sim_fw_sem)
ti <- lm(stab_com ~ richness + resp_div + env_stoch + ct_alive + w_avg_tlvl + Z +
  avg_int_strength + c, data = sim_fw_sem)
check_collinearity(ti)

library(DHARMa)
tar_load(model_fw_stab_comp)
tu <- simulateResiduals(model_fw_stab_comp$model[[3]])
plot(tu)
```
...
>   Last error: ℹ In argument: `data = map(...)`.
...
ggsave_multiple(
  ...
  units = "mm")
```
```{r fig-sem-tot-sensibility}
ggsave_multiple(
  paste0("p_sem_simplified_eff_tab_tot", c(".jpg", ".png", ".pdf")),
  plot = tar_read(p_sem_simplified_eff_tab_tot),
  path = here::here("report", "figures"),
  scale = 2.4,
  dpi = 400,
  width = 80,
...
tar_load(fixed_effect_fw_stab_comp)
