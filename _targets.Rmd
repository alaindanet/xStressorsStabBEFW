...
tar_read(sim) %>%
      ...
      select(richness, resp_div, cpe, cpe_int, cpe_env, sae_doak) %>%
      pivot_longer(cpe:sae_doak, names_to = "async_decomp", values_to = "values") %>%
      mutate(async_decomp = recode_factor(async_decomp, !!!var_replacement()))
tar_read(resp_div_async_decomp_prod) %>%
      ggplot(aes(x = as.factor(resp_div), y = values)) +
      geom_boxplot() +
      geom_hline(yintercept = 1) +
      ylim(c(min(resp_div_async_decomp_fw$values), max(resp_div_async_decomp_fw$values))) +
...
tar_read(p_resp_div_async_decomp)

    ggsave_multiple(
      paste0("p_resp_div_async_decomp_ok", c(".png", ".pdf")),
      plot = tar_read(p_resp_div_async_decomp),
      path = here::here("report", "figures"),
      scale = 2.4,
      dpi = 400,
      width = 80,
