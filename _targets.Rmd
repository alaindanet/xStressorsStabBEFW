...
map_tibble <- tibble::tibble(
  names = c("no_rerun", "non_allo_no_rerun", "rerun")
    )
```
...
```{targets rho_model}
list(
  tar_target(formula_fixed_rho, formula_mod_rho_fixed_effect()),
...
list(
  ...
  tar_target(sim_param, readRDS(here::here("data", "sim_param_d2"))),
...
```{r targets group-all-analysis}
list(
  tar_target(sem_simplified_eff_tab_tot,
    tibble(
      sim_type = map_tibble$names,
      data = list(sem_simplified_eff_tab_no_rerun,
        sem_simplified_eff_tab_non_allo_no_rerun, sem_simplified_eff_tab_rerun)
      ) %>%
    unnest(cols = data)),
  tar_target(p_sem_simplified_eff_tab_tot,
    sem_simplified_eff_tab_tot %>%
...
```{r, eval = FALSE}
tar_load(c(sem_simplified_collinearity_no_rerun, sem_simplified_collinearity_rerun,
    sem_simplified_collinearity_non_allo_no_rerun))
sem_simplified_collinearity_no_rerun %>%
  print(n = 30)
sem_simplified_collinearity_rerun %>%
  print(n = 30)
sem_simplified_collinearity_non_allo_no_rerun %>%
  print(n = 30)
```
...
```{r, eval = FALSE}
tar_load(c(sem_simplified_eff_tab_no_rerun, sem_simplified_eff_tab_rerun,
    sem_simplified_eff_tab_non_allo_no_rerun))
sem_simplified_eff_tab_no_rerun %>%
  filter(effect_type == "total", response %in% c("pop_stab"))
sem_simplified_eff_tab_rerun %>%
  filter(effect_type == "total", response %in% c("pop_stab"))
sem_simplified_eff_tab_non_allo_no_rerun %>%
  filter(effect_type == "total", response %in% c("pop_stab"))
```
...
```{r, eval = FALSE}
tar_load(sem_simplified_eff_tab_tot)
...
library(tidyverse)
library(magrittr)
tar_load(c(sim_fw_no_rerun, sim_fw_rerun))
sim_id <- intersect(sim_fw_no_rerun$sim_id, sim_fw_rerun$sim_id)
cor(
  sim_fw_no_rerun[sim_fw_no_rerun$sim_id %in% sim_id, ]$stab_com, sim_fw_rerun[sim_fw_rerun$sim_id %in% sim_id, ]$stab_com, use = "pairwise.complete.obs"
)
cor(
  sim_fw_no_rerun[sim_fw_no_rerun$sim_id %in% sim_id, ]$richness, sim_fw_rerun[sim_fw_rerun$sim_id %in% sim_id, ]$richness, use = "pairwise.complete.obs"
)
cor(
  sim_fw_no_rerun[sim_fw_no_rerun$sim_id %in% sim_id, ]$async, sim_fw_rerun[sim_fw_rerun$sim_id %in% sim_id, ]$async, use = "pairwise.complete.obs"
)
cor(
  sim_fw_no_rerun[sim_fw_no_rerun$sim_id %in% sim_id, ]$stab_pop, sim_fw_rerun[sim_fw_rerun$sim_id %in% sim_id, ]$stab_pop, use = "pairwise.complete.obs"
)
```
...
```{targets sem_tot_predictor, tar_simple = TRUE}
...
```{r}
p_pred_sr_resp_div_supp <- pred_stab_comp_resp_div %>%
  mutate(
    variable = var_replacement()[variable],
    response = recode_factor(response, !!!var_replacement(), .ordered = TRUE),
    ) %>%
...
sim_param %<>%
  select(-A)
sim_param %>%
  group_by(h, rho, env_stoch, Z) %>%
  summarise(
    nb_fw = length(unique(fw_id)),
    n = length(fw_id)
  )
