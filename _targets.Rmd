...
list(
  ...
  tar_target(tab_sem_vif,
    sem_collinearity %>%
      ...
      kable(
        caption = "Variance Inflation Factor for the Structural Equation Model.") %>%
      kable_styling()
    ),
...
list(
  ...
  tar_target(sem_file, here("report", "figures", "sem_stab_main_text.pdf"), format = "file"),
  tar_target(p_sem,
    ggdraw() +
...
sem_main_text_eff_tab %>%
  filter(effect_type == "direct",
    predictor %in% c(
      sem_tot_predictor(),
      "async", "stab_com", "pop_stab", "sae_total", "cpe", "evenness_sae", "sae_even"
      )) %>%
...
list(
  ...
  tar_target(tab_model_fw_stab_comp_wo_int_vif,
    model_fw_stab_comp_wo_int %>%
      ...
      select(Term, VIF) %>%
      kable(
        caption = "Variance Inflation Factor for the linear model predicting the
        relationship between stability and richness.") %>%
        kable_styling()
      ),
