      ...
      left_join(
        ...
        left_join(
          tibble(
            variable = c("avg_int_strength", "w_avg_tlvl", "ct_alive"),
            values = list(
              seq(0.01, 0.2, length.out = 50),
              seq(1.5, 3.5, length.out = 50),
              seq(.02, .3, length.out = 50)
            )
          )
        )
        ) %>%
...
list(
  ...
  tar_target(p_cpe_rho,
    sim_fw %>%
      ggplot(aes(x = rho, y = log(cpe), group = rho)) +
      geom_boxplot() +
      labs(
        x = var_replacement()["rho"],
        y = var_replacement()["cpe"]
        ) +
      theme_bw()
    ),
  tar_target(p_sem_bivariate_relations,
    plot_grid(
      ...
      p_sae_total_avg_int, p_cpe_avg_int, p_cpe_rho
    )
    )
)
```
...
tar_read(sem_simplified_eff_tab) %>%
  filter(effect_type == "total") %>%
  filter(response %in% c("async", "pop_stab", "stab_com")) %>%
  filter(!predictor %in% c("rho", "env_stoch","cpe", "sae_total", "evenness_sae", "sae_even", "cpe_int", "cpe_env", "pop_stab", "async", "S", "ct")) %>%
  mutate(across(c(response, predictor), ~var_replacement()[.x])) %>%
  ggplot(aes(
      x = predictor,
      y = effect,
...
```{r}
ggsave_multiple(
  paste0("p_sem_bivariate_relations", c(".png", ".pdf")),
  plot = tar_read(p_sem_bivariate_relations),
...
```{r}
library(ggbreak)
cols <- c("Community stability" = "black", "Asynchrony" = "blue", "Population stability" = "darkgreen")
alpha <- c("Community stability" = 1, "Asynchrony" = .3, "Population stability" = .3)
p_sem_tot_stab <- tar_read(sem_simplified_eff_tab) %>%
  filter(effect_type == "total") %>%
  filter(response %in% c("async", "pop_stab", "stab_com")) %>%
  filter(!predictor %in% c("cpe", "sae_total", "evenness_sae", "sae_even", "cpe_int", "cpe_env", "pop_stab", "async", "S", "ct")) %>%
  mutate(across(c(response, predictor), ~var_replacement()[.x])) %>%
  ggplot(aes(
      y = predictor,
      x = effect,
  ...
  scale_x_break(c(-0.3, -0.6))
p_sem_tot_stab_leg <- plot_grid(
  print(p_sem_tot_stab + theme(legend.position = "none")),
...
```{r}
tar_load(pred_stab_comp_rho)
p_pred_sr_rho <- pred_stab_comp_rho %>%
  filter(response == "stab_com") %>%
  mutate(
    variable = var_replacement()[variable],
    resp_div = 1 - rho
  ...
  coord_cartesian(expand = FALSE) +
  scale_fill_gradient2(
    low = rho_cat_colors()["Low"],
    mid = "white",
    high = rho_cat_colors()["High"],
    midpoint = 0,
    space = "Lab",
    na.value = "grey50",
    guide = "colourbar",
...
```{r}
tar_load(pred_data)
p_pred_sr_rho_int_env <- pred_data %>%
  filter(
    ct_alive %in% c(.1),
    rho %in% c(0, .75),
    richness >= 1,
  ...
  geom_ribbon(aes(ymin = stab_com - stab_com_se, ymax = stab_com + stab_com_se), alpha = .3, color = NA) +
  scale_alpha_manual(values = c(1, 1), guide = "none") +
  facet_grid(
    cols = vars(env_stoch),
    labeller = as_labeller(function(x) paste0("Environmental stochasticity: ", x))) +
  labs(
    x = "Species richness (Log)", y = "Community stability (Log)",
    linetype = "Avg. Interaction strength",
    color = "Environmental correlation", fill = "Environmental correlation"
  ) +
  theme_bw() +
  theme(legend.position = "bottom")
...
pred_data_sr_fw <- pred_data %>%
  filter(
    ...
    avg_int_strength %in% c(.01, .1),
    env_stoch == .1,
    w_avg_tlvl %in% c(2.0, 3.0)
    ) %>%
mutate(
  group_ct_alive = case_when(
    ct_alive == .3 & avg_int_strength == .01 & w_avg_tlvl == 2.0 ~ "High",
    ct_alive == .1 & avg_int_strength == .01 & w_avg_tlvl == 2.0 ~ "Low"
    ),
  group_avg_int_strength = case_when(
    ct_alive == .1 & avg_int_strength == .1 & w_avg_tlvl == 2.0 ~ "High",
    ct_alive == .1 & avg_int_strength == .01 & w_avg_tlvl == 2.0 ~ "Low"
    ),
  group_w_avg_tlvl = case_when(
    ct_alive == .1 & avg_int_strength == .01 & w_avg_tlvl == 3.0 ~ "High",
    ct_alive == .1 & avg_int_strength == .01 & w_avg_tlvl == 2.0 ~ "Low"
    )) %>%
pivot_longer(starts_with("group_")) %>%
filter(!is.na(value)) %>%
mutate(name = str_remove(name, "group_"))
p_pred_data_sr_fw <- pred_data_sr_fw %>%
  ...
  geom_ribbon(
    ...
    alpha = .3, color = NA) +
  scale_color_manual(values = rho_cat_colors()) +
  scale_fill_manual(values = rho_cat_colors()) +
