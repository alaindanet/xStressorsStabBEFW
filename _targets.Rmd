...
sim_for_plot %<>%
  select(-c(avg_cv_sp, sync)) %>%
  left_join(
    tar_read(sim_fw) %>%
      select(sim_id, ct_alive)
  )
```
```{r}
sim_for_plot %>%
  filter(env_stoch == .6, h == 2, Z == 1, S == 10, rho == 0) %>%
  arrange(desc(async))
...
```{r}
sim_for_plot %>%
  filter(env_stoch == .6, h == 2, Z == 100, S == 10, rho == 0) %>%
  arrange(desc(async))
high_z_low_rich <- sim_for_plot %>%
  filter(env_stoch == .6, h == 2, Z == 100, S == 10, rho == 0) %>%
  arrange(desc(async))

plot_timeseries_lg_tbl(timespecies_mat_to_long_tibble(high_z_low_rich$species[[1]]))
```

```{r}
sim_for_plot %>%
  filter(env_stoch == .6, h == 2, Z == 100, S == 40, rho == 0) %>%
  arrange(desc(async))
high_z_high_rich <- sim_for_plot %>%
  filter(env_stoch == .6, h == 2, Z == 100, S == 40, rho == 0) %>%
  arrange(desc(async))
plot_timeseries_lg_tbl(timespecies_mat_to_long_tibble(high_z_high_rich))
```

```{r}
low_z_high_rich <- sim_for_plot %>%
  filter(env_stoch == .6, h == 2, Z == 1, S == 40, rho == 0) %>%
  arrange(desc(async))

low_z_high_rich_mat <- get_mat_list(low_z_high_rich)
plot_timeseries_lg_tbl(timespecies_mat_to_long_tibble(low_z_high_rich_mat[[1]]))
```
...
p_high_z_low_rich <- plot_sim(
  x = sim_for_plot,
  id = high_z_low_rich$sim_id[[1]],
  col_palette = NULL,
  node_size_scale = .3)
p_high_z_high_rich <- plot_sim(
  x = sim_for_plot,
  id = high_z_high_rich$sim_id[[1]],
  col_palette = NULL,
  node_size_scale = .4
)

p_sim_example <- plot_grid(
  p_low_z_low_rich, p_low_z_high_rich,
  p_high_z_low_rich, p_high_z_high_rich,
  labels = c("b", "", "", ""),
  align = "hv"
)
...
```{r}
sim_for_plot %>%
  filter(Z == 100, env_stoch == 0.6, rho %in% c(0, 1), S == 10, h == 2) %>%
  group_by(fw_id) %>%
  summarise(n = n()) %>%
  filter(n > 1)
sim_for_plot %>%
  filter(env_stoch == 0.6, rho %in% c(0, .75), S == 10, h == 3) %>%
  group_by(fw_id) %>%
  summarise(n = n()) %>%
  filter(n > 1)
fwid <- 549
...
p_sim_rho_example <- plot_grid(
  p_low_resp_div_low_rich, p_high_resp_div_low_rich,
  labels = c("a", ""),
  align = "hv"
)
ggsave_multiple(
...
```{r}
tar_load(pred_data)
p_pred_sr_resp_div_int_env <- pred_data %>%
  filter(
    ct_alive %in% c(.1),
    resp_div %in% c(0, .75),
    richness >= 1,
