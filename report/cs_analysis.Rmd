...
- Mainly done in plants
- Food-webs structure can also modulate synchrony by predator-prey relationships
- Interaction strength, connectance, richness and enrichment can also drive more
...
- Important bits:
  ...
  - Standardize carrying capacity? (Remove the effect of species richness on
...
ggsave(
  filename = here::here("fig", "stability_rich_fw_only.pdf"),
  p_sim_rich_prod,
  scale = 1.8,
  width = 88,
  height = 88 * .6,
...
ggsave(
  filename = here::here("fig", "stability_rich_p_only.pdf"),
...
# CV bagged model
bagged_cv <- train(
  stab_com ~ richness + ct_alive + rho + productivity + max_tlvl_alive +
    avg_max_int_alive,
  data = simZ,
  cp = .2,
  maxdepth = 6,
...
```{r}
m1 <- rpart(
  formula = stab_com ~ richness + ct_alive + rho + productivity + max_tlvl_alive +
    avg_max_int_alive,
  data    = simZ,
  method  = "anova"
  )
